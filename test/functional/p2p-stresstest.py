...
import time, threading, traceback, http
...
class StressTest(BitcoinTestFramework):
    ...
    def generate_spam(self, value, txcount):
        ...
        for i in range(2):
            ...
            self.nodes[0].generate(1)
            t2b = time.time(); print("took %3.3f sec" % (t2b-t2a))
            for i in range(self.num_nodes):
                while self.nodes[i].getblockcount() == oldheight:
                    time.sleep(0.05)
                t2c = time.time(); print("%i:%6.3f   " % (i, t2c-t2b), end="")
            print()
            sync_blocks(self.nodes, timeout=180)
            t2c = time.time(); print("Syncing block took %3.3f sec" % (t2c-t2b))
